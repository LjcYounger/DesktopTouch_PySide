...
def ChangeImageByGrayscale(image, color, alpha, extent, trans):
     ...
     newalpha = grayscale * alpha // extent
     img_array[:, :, 3] = newalpha
     resultbytes = img_array.tobytes()
     resultimage = pg.image.frombuffer(resultbytes, (image.get_size()[1], image.get_size()[0]), "RGBA")
     return resultimage
...
def apply_glow_effect(surface, thickness, blur_sigma):
    ...
    resultbytes = glow_array.tobytes()
    glow_surface = pg.image.frombuffer(resultbytes, (height, width), "RGBA")
